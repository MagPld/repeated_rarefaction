...
test_threshold_test <- function(input, repeats = 10, t_min = 50, t_max = 250, t_step = 1, group = "sample_id") {
  ...
  for (y in repeats) {
    for (x in thresholds) {
      ...
      # Calculate index
      index <- clusterSim::index.G1(just_positions, cluster_numbers)
      index_consensus <- clusterSim::index.G1(just_positions_consensus, cluster_consensus_numbers)
      
      index_data <- rbind(index_data, c(toString(y), x, index, index_consensus))

    }
  }
  
  colnames(index_data) <- c("Repeat_Amount", "Threshold", "Index", "Index_consensus")
  index_data <- as.data.frame(index_data)
  index_data$Threshold <- as.numeric(index_data$Threshold)
  index_data$Index <- as.numeric(index_data$Index)
  ...
  return(list(index_plot = index_plot, index_consensus_plot = index_consensus_plot, ordination_plots = plots))
}
